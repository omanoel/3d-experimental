...
export class ThreeMapComponent {
  ...
  @Input() model: ThreeComponentModel | undefined;
  ...
  resetWidthHeight(): void {
    this._threeComponentService.resetWidthHeight(
      this.model as ThreeComponentModel,
      window.innerWidth,
      window.innerHeight
    );
  }
  ...
  onMousemove(event: MouseEvent): void {
    ...
    if (!this._mouseDown) {
      (this.model as ThreeComponentModel).mouse.x =
        (event.clientX / window.innerWidth) * 2 - 1;
      (this.model as ThreeComponentModel).mouse.y =
  ...
  onClick(event: MouseEvent): void {
    event.preventDefault();
    this._threeComponentService.gotoTarget(this.model as ThreeComponentModel);
  }
}
