...
import { PolarRepositoryModel } from './polar-repository.model';

@Injectable({
  providedIn: 'root',
})
export class PolarRepositoryService {
  private static readonly COUNT = 20;
  private static readonly FACTOR = 4;

  private _model: PolarRepositoryModel | undefined;
  ...
  public get model(): PolarRepositoryModel {
    return this._model as PolarRepositoryModel;
  }
  ...
  public update(): void {
    ...
    if (
      dist >
      (this._model as PolarRepositoryModel).distReference *
        (PolarRepositoryService.FACTOR - 1)
    ) {
      (this._model as PolarRepositoryModel).distReference = dist;
      this._updateObjects();
    } else if (dist < (this._model as PolarRepositoryModel).distReference) {
      (this._model as PolarRepositoryModel).distReference =
        dist / (PolarRepositoryService.FACTOR - 1);
      this._updateObjects();
    }
  }

  private _updateObjects(): void {
    for (const ellipse of (this._model as PolarRepositoryModel).objects) {
      this._sceneService.model.remove(ellipse);
    }
    const color = 0xffffff;
    (this._model as PolarRepositoryModel).objects = this._buildObjects(
      color,
      (this._model as PolarRepositoryModel).distReference
    );
    for (const ellipse of (this._model as PolarRepositoryModel).objects) {
  ...
  private _buildEllipsesOnXY(
    ...
    for (let i = 1; i < PolarRepositoryService.COUNT; i++) {
      const radius =
        (distance * i * PolarRepositoryService.FACTOR) /
        PolarRepositoryService.COUNT;
